...
Карта, аренды, расчёты, симулятор и почта пока не реализованы; их контракты описаны как planned.
...
- Worker — отдельный процесс: он доставляет задания из очереди `outbox`, снимает истёкшие брони (раз в секунду), удаляет доставленные сигналы старше суток и результаты команд старше 24 часов. Очередь переживает остановку процесса: задание получает lease на 30 секунд, повторяет попытки с задержкой от 1 до 60 секунд и не отмечается успешным, пока не доставлено.
...
следов у отката. Наборы намеренно ждут реального времени и занимают несколько минут.
...
Обслуживаются health, операции учётной записи, публичный каталог и оба потока сигналов.
