...
// CursorHMACKeyFileVariable names the file holding the key the signed pagination cursors of this
// process are issued under. It is generated by local setup like the other capability material, and
...
func CursorSigningKey() ([]byte, error) {
	...
	if os.Getenv(CursorHMACKeyFileVariable) != "" && key == "" {
		return nil, errors.New(CursorHMACKeyFileVariable + " names an empty cursor signing key")
	}
	return []byte(key), nil
}
