	...
	for _, request := range []struct {
	...
	}{
		{
			...
			check: func(command rentals.DemoCommand) error {
				return expecting(command, rentals.SetTelemetryState, vehicleID, false, "")
			},
		},
		{
			...
			check: func(command rentals.DemoCommand) error {
				if err := expecting(command, rentals.SetPosition, vehicleID, false, ""); err != nil {
					return err
				}
		...
		},
		{
			...
			check: func(command rentals.DemoCommand) error {
				if err := expecting(command, rentals.SetEnergyRemaining, vehicleID, false, ""); err != nil {
					return err
				}
		...
		},
		{
			...
			check: func(command rentals.DemoCommand) error {
				return expecting(command, rentals.MarkServiced, vehicleID, false, "")
			},
		},
		{
			...
			check: func(command rentals.DemoCommand) error {
				if err := expecting(command, rentals.SetNextPaymentOutcome, "", false, rentalID); err != nil {
					return err
				}
...
// Every action the internal surface applies becomes a command the module knows: the vocabulary and the
// module name the same actions, so a row added to one without the other is a failing test rather than
// an action that reaches the module only to be refused there.
func TestEveryDemonstrationActionBecomesACommandTheModuleKnows(t *testing.T) {
	for _, action := range democontrol.Actions() {
		...
		t.Run(action.Name, func(t *testing.T) {
			command, err := demoCommand(democontrol.Stated{
				ActionID: "01994342-6ba7-7000-8000-000000000001",
				Action:   action.Name,
				Values:   statedFields(action),
			})
			if err != nil {
				t.Fatalf("the %s action could not be read: %v", action.Name, err)
			}
			if command.Kind != rentals.DemoActionKind(action.Name) {
				t.Fatalf("the action became the kind %q", command.Kind)
			}
