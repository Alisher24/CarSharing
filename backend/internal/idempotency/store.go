...
// ClaimKey takes the key of a command for this attempt, or reports the answer the key already holds.
//
...
func ClaimKey(
	ctx context.Context, tx pgx.Tx, owner Owner, key Key, fingerprint Fingerprint,
) (Claim, error) {
	querier := tx
	...
	return stored(ctx, tx, owner, key, fingerprint)
}
...
func stored(
	...
	var answer Result
	err := tx.QueryRow(ctx, storedResultStatement,
...
func Complete(ctx context.Context, tx pgx.Tx, owner Owner, key Key, answer Result) error {
	_, err := tx.Exec(ctx, completeStatement,
