...
/** The unit an amount is written with, which is the som and not `KGS`. */
export const SOM_UNIT = 'сома';
...
export function somText(tyiyn: string): string | undefined {
  ...
  return `${somFormat.format(amount / TYIYN_IN_A_SOM)}${DECIMAL_MARK}${minor} ${SOM_UNIT}`;
}
