...
import type { Completion, Progress, SourceKind } from '../api/current.ts';
import { commandText, type CommandPhase } from '../command/commandPhase.ts';
import { amountText, COMPLETION_UNKNOWN, UNREADABLE_VALUE } from './fares.ts';
import {
  completionText,
  FINISH_ACTION,
  FINISH_QUESTION,
...
describe('what a ride costs so far', () => {
  test('is the estimate the service published, divided into som exactly', () => {
    assert.equal(amountText(progress('1234').estimated_amount_tyiyn), '12,34 сома');
    assert.equal(amountText(progress('0').estimated_amount_tyiyn), '0,00 сома');
  });
  ...
  test('keeps every tyiyn of an amount larger than a floating-point number can hold', () => {
    ...
    const amount = amountText(progress('900719925474099399').estimated_amount_tyiyn);
  ...
  test('is written as missing rather than as zero when the amount cannot be read', () => {
    assert.equal(amountText(progress('не число').estimated_amount_tyiyn), '—');
    assert.equal(amountText(progress('-1').estimated_amount_tyiyn), '—');
  });
});
...
describe('what a completed ride is reported with', () => {
  test('writes the total of the invoice as som, exactly as the invoice states it', () => {
    assert.equal(amountText('2789'), '27,89 сома');
    assert.equal(amountText('900719925474099399').replaceAll('\u00a0', ' '), '9 007 199 254 740 993,99 сома');
    assert.equal(amountText('не число'), UNREADABLE_VALUE);
  });
